...
def standardize_activation_key_data(key_data):
    """
...
def list_activation_keys(client, org_id=None):
    ...
    if org_id:
        path = f"/activationkey/listActivationKeys?orgId={org_id}"
    else:
    ...
    return [standardize_activation_key_data(key) for key in keys if isinstance(key, dict)]
...
def get_activation_key_details(client, key_id=None, key_name=None):
    ...
    try:
        ...
        if key_id is not None:
            ...
            if key:
                return standardize_activation_key_data(key)
...
def create_activation_key(module, client):
    ...
    if module.check_mode:
        return (
            True,
            {"key": key_name},
            f"Activation key '{key_name or 'autogenerated'}' would be created",
        )

    # Create the activation key using SUSE Manager API
    ...
    return (
        True,
        result,
        f"Activation key '{key_name or 'autogenerated'}' created successfully",
    )
...
def update_activation_key(module, client):
    ...
    if entitlements is not None:
        ...
        if sorted(current_entitlements) != sorted(entitlements):
            details_struct["entitlements"] = entitlements
            changes_needed = True
...
def manage_activation_key_channels(module, client):
    ...
    if channel_state == "present":
        ...
        try:
            ...
            result = client.post(add_path, data={"key": actual_key_name, "channelLabels": channels_to_add})
        ...
        try:
            ...
            result = client.post(remove_path, data={"key": actual_key_name, "channelLabels": channels_to_remove})
...
def manage_activation_key_packages(module, client):
    ...
    if current_packages_objs:
        ...
        current_packages = [pkg.get("name") for pkg in current_packages_objs if isinstance(pkg, dict) and "name" in pkg]
    ...
    if package_state == "present":
        ...
        try:
            ...
            result = client.post(add_path, data={"key": actual_key_name, "packages": package_objects})
        ...
        try:
            ...
            result = client.post(remove_path, data={"key": actual_key_name, "packages": package_objects})
